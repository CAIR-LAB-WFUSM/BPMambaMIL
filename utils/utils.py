...
def calculate_instance_scores(data, label):
    """
    计算每个实例的分数
    data: tensor of shape (n_instances, n_features)
    label: tensor of shape (1,)
    """
    # 使用简单的注意力机制计算分数
    attention = torch.nn.Linear(data.shape[1], 1)
    scores = F.softmax(attention(data).squeeze(), dim=0)
    return scores
...
def get_simple_loader(dataset, batch_size=1, num_workers=1):
	kwargs = {'num_workers': 1, 'pin_memory': False, 'num_workers': num_workers} if device.type == "cuda" else {}
...
def get_split_loader(split_dataset, training = False, testing = False, weighted = False):
	...
	kwargs = {'num_workers': 1} if device.type == "cuda" else {}
	 # Set the training flag for the dataset
	split_dataset.set_training(training)
	if not testing:
		if training:
			if weighted:
		...
		# Select random size test inputs for validation
		loader = DataLoader(split_dataset, batch_size=1, sampler=SequentialSampler(split_dataset), collate_fn=collate_MIL, **kwargs)
